...
class CommandThread (threading.Thread):
	...
	def initialize(self):
		...
		if code == 255:
			self.logger.info("unable to connect")
			return
		# upload initialize.sh
		c = 'scp initialize.sh %(address)s:~/'
		r, code = run_command(
			c % {'address': self.config['address'], 'server': self.config['server']})
		self.logger.debug(r)
		self.logger.debug(code)
...
python3 cluster.py COMMAND
	...
	track:
		start tracking users through a location in real time
...
if __name__ == '__main__':
	...
	if not os.path.exists(args.config):
		...
		if len(config['server']) == 0:
			config['server'] = 'http://192.168.1.8:3000'
		if 'http' not in config['server']:
